...
export default function Edit( props ) {
	...
	useEffect( () => {
		if ( languageLabel == 'no' ) {
			...
			setAttributes( { showLanguageLabel: true } );
		} else {
			let tmpLabelClass = [];
	...
	if ( [ 'htmlmixed', 'javascript', 'xml', 'jsx', 'vue' ].includes( mode ) ) {
		...
		<div { ...useBlockProps() }>
			<Fragment>
				<InspectorControls>
					<PanelBody
						initialOpen={ false }
						title={ __( 'CodeMirror Panel Settings' ) }
					>
						<ToggleControl
							label={ __( 'Show Panel' ) }
							checked={ showPanel }
							onChange={ () =>
								setAttributes( { showPanel: ! showPanel } )
							}
						/>
						<ToggleControl
							label={ __( 'Show Language Label' ) }
						...
						/>
						{ showPanel && showLanguageLabel && (
							<SelectControl
								label={ __( 'Langrage Label' ) }
								value={ languageLabel }
								help={ __(
				...
				<div className="codeMirror-editor">
					...
					>
						...
						>
							...
							{ /* {languageLabel === 'file' ? <RichText
								...
								autoFocus={false}
							/> :
								<span className={makeClassName(languageLabelClass)}>{languageLabelText}</span>
							} */ }
							<div className="control-panel">
								...
								{ showFullScreenButton && (
									<span
										...
										onClick={ () =>
											displayAlert(
												'To Set Full Screen on Front End'
											)
										}
									>
					...
					<CodeMirrorEditor
						...
						showPanel={ showPanel && showLanguageLabel }
