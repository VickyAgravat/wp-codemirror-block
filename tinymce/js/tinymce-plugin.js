...
( function () {
	...
	var plugin = function ( editor, url ) {
		...
		var isCodeMirrorBlock = function ( node ) {
			return (
				node.nodeName == 'PRE' &&
				editor.dom.hasClass( node.parentElement, 'code-block' )
			);
		};
        ...
        */
		...
		var codeInsertAction = function () {
			...
			var dialogWidth =
				Math.min( editorDimensions.w, window.innerWidth ) || 700;
			var dialogHeight =
				Math.min( editorDimensions.h, window.innerHeight ) || 500;

			// create the dialog
			Dialog.open(
				codeInsertDialog( dialogWidth, dialogHeight ),
				function ( e ) {
		...
		var getCodeBlockSettings = function ( node, inlineMode ) {
			...
			if ( blockSetting ) {
				...
				config = {
					...
					mime:
						blockSetting.mime !== undefined
							? blockSetting.mime
							: _defaults.mime,
					theme:
						blockSetting.theme !== undefined
							? blockSetting.theme
							: _defaults.theme,
					lineNumbers:
						blockSetting.lineNumbers !== undefined
							? blockSetting.lineNumbers
							: _defaults.lineNumbers,
					styleActiveLine:
						blockSetting.styleActiveLine !== undefined
							? blockSetting.styleActiveLine
							: _defaults.styleActiveLine,
					lineWrapping:
						blockSetting.lineWrapping !== undefined
							? blockSetting.lineWrapping
							: _defaults.lineWrapping,
					// readOnly: _readOnly,
					readOnly:
						blockSetting.readOnly !== undefined
							? blockSetting.readOnly
							: _defaults.readOnly,
					// disableCopy: (blockSetting.readOnly !== undefined && blockSetting.readOnly == 'nocursor') ? true : false,
					align:
						blockSetting.align !== undefined
							? blockSetting.align
							: _defaults.align,
				};
			} else {
